...
let isWakeModeActive = false;
let wakePollInterval = null;
...
async function startChat() {
  ...
  if (uploadedAudioFile) {
    ...
    try {
      ...
      fd.append('voice_type', profile.voiceType || 'female');
...
async function speakText(text, btnEl) {
  ...
  try {
    ...
    // Update clone badge based on response
...
async function toggleWakeMode() {
  ...
  if (isWakeModeActive) {
    ...
    if (wakePollInterval) {
      clearInterval(wakePollInterval);
      wakePollInterval = null;
    }
    showToast(uiLang === 'hi' ? 'Wake mode band ho gaya' : 'Wake mode disabled');
  } else {
    // Start wake mode
    isWakeModeActive = true;
    wakeBtn.classList.add('listening');
    showToast(uiLang === 'hi' ? 'Aira sun rahi hai... bolo "Aira"' : 'Aira is listening... say "Aira"');

    // Poll wake_status endpoint
    if (wakePollInterval) clearInterval(wakePollInterval);
    wakePollInterval = setInterval(async () => {
      try {
        const res = await fetch('/api/wake_status');
        const data = await res.json();
        if (data.detected) {
          // Wake word detected! Start recording
          console.log('[Wake] Wake word detected!');
          showToast(uiLang === 'hi' ? 'Aira jaag gayi! Bolo...' : 'Aira woke up! Speak now...');
          await startRecording();
        }
      } catch (e) {
        console.error('[Wake] Poll error:', e);
      }
    }, 500);
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Click anywhere on intro screen to skip (except the enter button)
