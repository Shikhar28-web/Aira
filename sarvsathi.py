"""
SarvSathi — Standalone Voice Assistant
=======================================
Pure voice pipeline (no browser needed):

    Say "SarvSathi"  →  Speak your command  →  Hear the response
...
Optional env vars (same as server.py):
    SARVSATHI_DEVICE   cuda | cpu     (default: cuda)
...
def _play_wav_b64(audio_b64: str) -> None:
    """Decode a Base64 WAV string and play it via sounddevice."""
    try:
...
def main() -> None:
    print("\n" + "=" * 60)
    print("  SarvSathi — Offline Voice Assistant (standalone)")
    ...
    print("  Say  'SarvSathi'  to start a conversation.")
    ...
    while True:
        try:
            ...
            wake.detected.clear()
            wake.start()
            print("[SarvSathi] Waiting for wake word …")
            wake.detected.wait()
            wake.stop()

            # ── Phase 2: Record the user's command ─────────────────────────
            print("[SarvSathi] Wake word heard! Listening …")
            audio_np = _record_until_silence(_SAMPLE_RATE)

            if audio_np is None or len(audio_np) == 0:
                print("[SarvSathi] No audio captured. Try again.\n")
                continue
            ...
            if not user_text:
                print("[SarvSathi] Couldn't understand. Please try again.\n")
                continue
            ...
            print("\n\n  [SarvSathi] Shutting down. Alvida! 👋\n")
            break
